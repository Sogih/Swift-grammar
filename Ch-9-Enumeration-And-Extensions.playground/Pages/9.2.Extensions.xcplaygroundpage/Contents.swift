 ...
 * 코드 정리
    * 익스텐션으로 메소드 정의
        * 기존의 클래스 정의와 구분되어 점프 바에서 별도의 그룹으로 표시됨
        * 이를 활용하여 오버라이드 메소드와 커스텀 메소드를 구분하는 등 효율적으로 코드를 정리할 수 있음
        * 익스텐션에서는 오버라이드 메소드를 작성하지 못함
        * 익스텐션에서는 @IBAction 메소드는 작성하지 못함
    * 익스텐션으로 델리게이트 메소드 정의
    * MARK 주석
        * //MARK: (주석 내용)
        * 점프 바나 심벌 탐색기에 표시됨
        * 콜론(:) 뒤에 하이픈(-) 하나 붙여주면 점프 바에서 마크 주석 바로 위에 가로줄 생성
*/
